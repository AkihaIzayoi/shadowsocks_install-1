...
#git安装ssr
git clone https://github.com/w123456w30w/shadowsocksr-1.git ${ssrdir}
cd ${ssrdir}
...
if [ "$OS" == 'CentOS' ]; then
	if ! wget --no-check-certificate https://raw.githubusercontent.com/w123456w30w/shadowsocks_install/master/ssr -O /etc/init.d/ssr; then
		echo "Failed to download ssr chkconfig file!"
		exit 1
	fi
else
	if ! wget --no-check-certificate https://raw.githubusercontent.com/w123456w30w/shadowsocks_install/master/ssr-debian -O /etc/init.d/ssr; then
		echo "Failed to download ssr chkconfig file!"
		exit 1
	fi
fi
